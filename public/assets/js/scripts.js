...
function get_data_pagination(page = null) {
    ...
    history.pushState({}, "Results for Cats", "?" + search);
...
function auto_login_admin_manager(id){
    $.ajax({
...
function auto_login_manager_user(id){
    $.ajax({
        url: "/manager_action/auto_login_users",
        data: { enc_id:id },
        method: "POST",
        contentType: "application/x-www-form-urlencoded",
        success: function (rsp) {
...
function bounce_queue(e, p_id) {
    $.ajax({
        url: "/user_action/bounce_servers/queue_process",
        data: { queue_work: e.getAttribute("data-check"), enc_id: p_id },
        method: "POST",
        contentType: "application/x-www-form-urlencoded",
        success: function (rsp) {
            if (rsp.status == 1) {
                if (e.getAttribute("data-check") == "nonqueue") {
                    e.classList.remove("btn-success");
                    e.classList.add("btn-warning");
                    e.innerHTML = '<i class="fa fa-spinner fa-eercast"></i>';
                    e.setAttribute("data-check", "queue");
                } else {
                    e.classList.remove("btn-warning");
                    e.classList.add("btn-success");
                    e.innerHTML = '<i class="fa fa-eercast"></i>';
                    e.setAttribute("data-check", "nonqueue");
                }
            } else {
                alert(rsp.msg)
            }
